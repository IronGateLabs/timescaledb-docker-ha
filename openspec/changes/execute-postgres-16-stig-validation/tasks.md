...
Notes: added an optional `validation_state` field (`executed` | `candidate`) to the mapping and traceability schemas, propagated by `sync_traceability_from_mapping.py` and enforced by both guardrails (a `validation_only` control MUST declare a state; the field is rejected on any other status). Driven by the CI run: controls whose mapped legacy checks PASSED are `executed`; the rest are `candidate`. After the §5 image-config hardening, a second validation run promoted 7 more (V-261862/910/913/924/932/941/948), giving 39 executed / 21 candidate. The 21 remaining candidates are blocked by legacy-check portability issues to fix in the forked profile (the deferred upstream PR), not by the image. 7 controls were reclassified `validation_only` → `deployment_owned` (TLS/PKI/pg_hba/log-offload). `summarize_inspec_results.py` now de-duplicates the include_controls wrapper double-count and reports the executed/candidate split. New counts: validation_only 60 (39 executed / 21 candidate), deployment_owned 31, image_enforced 18, manual 2.
